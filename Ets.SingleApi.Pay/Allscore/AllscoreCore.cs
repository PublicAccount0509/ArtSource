...
namespace Ets.SingleApi.Pay.Allscore
{
    ...
    public class Core
    {
        ...
        public static string BuildMysign(Dictionary<string, string> dicArray, string key, string _input_charset)
        {
            string prestr = CreateLinkString(dicArray);  //把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串

            prestr = prestr + key;                      //把拼接后的字符串再与安全校验码直接连接起来
            string mysign = Sign(prestr,  _input_charset);	//把最终的字符串签名，获得签名结果
