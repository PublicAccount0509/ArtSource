...
namespace Ets.SingleApi.Services
{
    ...
    public class WaiMaiBaiFuBaoPayment : IPayment
    {
        ...
        public PaymentResult<string> Payment(IPaymentData parameter)
        {
            ...
            subpara.return_url = baiFuBaoPaymentQueryData.ReturnUrl;//后台通知
        ...
        /// 修改时间：
        /// ----------------------------------------------------------------------------------------
        private static bool VerifySignature(BaiFuBaoReturnParameter retrunData)
        {
        ...
        /// <returns></returns>
        private static string ComputeHash(string content, string sign_method)
        {
        ...
        /// 修改时间：
        /// ----------------------------------------------------------------------------------------
        private static string BuildRequestURL(SortedList<string, string> dictionary)
        {
            IList<string> buffer = new List<string>();
            foreach (string key in dictionary.Keys)
            {
                string value = dictionary[key];
                string segment = string.Format("{0}={1}", key, value);
                buffer.Add(segment);
            }
