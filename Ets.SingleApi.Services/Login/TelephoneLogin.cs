namespace Ets.SingleApi.Services
{
    ...
    public class TelephoneLogin : ILogin
    {
        ...
        {
            if (!this.loginEntityRepository.EntityQueryable.Any(p => p.Username == userName))
            {
            ...
            var loginIdList = this.loginEntityRepository.EntityQueryable.Where(p => p.Username == userName && p.Password == password).Select(p => p.LoginId).ToList();
